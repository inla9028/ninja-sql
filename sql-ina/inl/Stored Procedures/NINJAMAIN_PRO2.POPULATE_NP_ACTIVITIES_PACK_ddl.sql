...
-- Generated 01.06.2010 20:34:44 from NINJAMAIN_PRO2@ninjat
CREATE OR REPLACE 
...
-- Populate table for sending email to call center when no reply received on BEST or ENDR within 5 days.
-- inl 31.05.2010 TD4265 - Ninja should only send email and not resend BEST for these cases.
 PROCEDURE POPULATE_NP_BEST_RESEND IS
   ...
   where port_ind in ('Y','B','P','I')
     ...
     and trx_code in (105,108 )
     and trx_source='INT'
     and request_exec_date between (sysdate-25) and sysdate
     and nt.int_trx_seq = (select max(int_trx_seq )
			   from np_trx_detail@FOKUS nt2
			   where nt2.trx_code in (105,108 ) 
			     and nt2.int_order_id=np.int_order_id)
