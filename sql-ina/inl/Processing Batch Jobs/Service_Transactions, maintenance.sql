...
SELECT a.process_status, a.priority, COUNT(*) as "COUNT",
       TO_NUMBER(LTRIM(TO_CHAR((COUNT(*) * 8.81) / 3600, '9999999'))) || ' hours ' ||
       TO_NUMBER(LTRIM(TO_CHAR(MOD((COUNT(*) * 8.81) / 60, 60),   '9999999'))) || ' min' AS "QUEUE"
...
SELECT a.process_status, COUNT(*) as "COUNT",
       TO_NUMBER(LTRIM(TO_CHAR((COUNT(*) * 3.38) / 3600, '9999999'))) || ' hours ' ||
       TO_NUMBER(LTRIM(TO_CHAR(MOD((COUNT(*) * 3.38) / 60, 60),   '9999999'))) || ' min' AS "QUEUE"
  FROM ninjadata.service_transactions a
  WHERE a.process_status = 'WAITING'
...
SELECT a.trans_number, a.subscriber_no, a.soc, a.action_code,
       ...
       a.next_action_code, a.next_action_argument
  FROM ninjadata.service_transactions a
  WHERE a.subscriber_no IN ('GSM04746342018')
...
SELECT a.trans_number, a.subscriber_no, a.soc, a.action_code, a.process_time, a.process_status, a.status_desc
...
UPDATE ninjadata.service_transactions x
  SET x.process_status = 'TRANSFER', x.status_desc = 'DUPLICATE 2008.07.17', x.process_time = SYSDATE
...
--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==
--== Display the statistics of the current day, separating on streams ==--==--==
--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==
