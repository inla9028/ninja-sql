...
SELECT TO_CHAR(a.enter_time, 'YYYY-MM-DD HH24:MI') AS "ENTER_TIME",
       COUNT(*) AS "COUNT",
       TO_NUMBER(LTRIM(TO_CHAR(FLOOR((COUNT(*) * 1.410) / 3600), '9999999'))) || ' hours ' ||
       TO_NUMBER(LTRIM(TO_CHAR(MOD((COUNT(*) * 1.410) / 60, 60), '9999999'))) || ' min' AS "QUEUE"
...
--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==
SELECT NVL(a.dealer_code, '(N/A)') AS "DEALER_CODE", NVL(a.sales_agent, '(N/A)') AS "DEALER_CODE", COUNT(*) AS "COUNT"
  FROM ninjadata.service_transactions a
  WHERE a.enter_time > TRUNC(SYSDATE)
  GROUP BY NVL(a.dealer_code, '(N/A)'), NVL(a.sales_agent, '(N/A)') 
  ORDER BY NVL(a.dealer_code, '(N/A)'), NVL(a.sales_agent, '(N/A)');
...
--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==
--== Display the socs, actions & priorities for the socs for the last 14 days.
--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==
SELECT a.soc, a.action_code, a.priority, COUNT(*) AS "COUNT"
--     , TO_NUMBER(LTRIM(TO_CHAR(FLOOR((COUNT(*) * 1.410) / 3600), '9999999'))) || ' hours ' ||
--       TO_NUMBER(LTRIM(TO_CHAR(MOD((COUNT(*) * 1.410) / 60, 60), '9999999'))) || ' min' AS "QUEUE"
...
--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==
SELECT   "ENTER_TIME",
         TO_NUMBER (LTRIM (TO_CHAR (AVG ("DURATION"), '9999999.99'))
                   ) AS "AVG_DURATION"
    FROM (SELECT    TO_CHAR (a.enter_time, 'YYYY-MM-DD HH24')
                 || ':00-59' AS "ENTER_TIME",
                 24 * 60 * 60
                 * (a.process_time - a.request_time) AS "DURATION"
            FROM ninjadata.service_transactions a
