...
SELECT TO_NUMBER(a.stream) AS "STREAM", a.process_status, COUNT(*) AS "COUNT",
       TO_NUMBER(LTRIM(TO_CHAR(FLOOR((COUNT(*) * 3.466) / 3600), '9999999'))) || ' hours ' ||
       TO_NUMBER(LTRIM(TO_CHAR(MOD((COUNT(*) * 3.466) / 60, 60),   '9999999'))) || ' min' AS "QUEUE"
  FROM ninjadata.master_transactions a
  WHERE a.request_id = 'TRANS ' || TO_CHAR(SYSDATE, 'YYYY.MM.DD')
--    AND a.enter_time > TRUNC(SYSDATE)
...
--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==
--== List the processes currently working on ninjadata.master_transactions ...
--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==
