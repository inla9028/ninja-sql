...
/*
** Delete the feature mappings for the changed MBN_USER (the old soc MBNU01 was
** replaced with soc HPMBN02 which doesn't have any feature parameters).
*/
DELETE
  FROM spm_feature_mapping a
 WHERE a.sp_service_code = 'MBN_USER'
;
...
SELECT UNIQUE ssm.sp_code, 'maps to' AS "MAPS_TO", ssm.soc_type, ssm.soc_group
 ...
 WHERE sfm.sp_service_code      = ssm.sp_code
   ...
   AND s.soc                    = sts.soc
   AND sts.subscription_type_id = 'PW20' || 'REG1'
   AND SYSDATE            BETWEEN sts.effective_date AND NVL(sts.expiration_date, SYSDATE + 1)
   AND sts.soc                  = sd.soc
