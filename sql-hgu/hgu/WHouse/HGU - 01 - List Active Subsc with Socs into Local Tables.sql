...
SELECT a1.ban, a1.subscriber_no, rtrim(a1.soc) soc
       ...
       , 'CUG' AS "PARAM1"
       , RTRIM(SUBSTR(a1.ftr_add_sw_prm, INSTR(a1.ftr_add_sw_prm, 'CUG=') + 4, 5)) AS "VALUE1"
       , a2.feature_code AS "FTR2"
       , 'VPN' AS "PARAM2"
       , RTRIM(SUBSTR(a2.ftr_add_sw_prm, INSTR(a2.ftr_add_sw_prm, 'VPN=') + 4, 5)) AS "VALUE2"
...
/*
** Some CUG values are apparently less than 5 characters, despite that being
** a business-rule. Fix this by checking for cases of '@' in the param-values.
*/
UPDATE tmp_hgu_ftr_current fc1
   SET fc1.value1 = SUBSTR(fc1.value1, 0, INSTR(fc1.value1, '@') - 1)
 WHERE 1 = 1
   AND INSTR(fc1.value1, '@') != 0
;
