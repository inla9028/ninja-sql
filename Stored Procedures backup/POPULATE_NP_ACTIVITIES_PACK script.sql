DROP PACKAGE POPULATE_NP_ACTIVITIES_PACK;

CREATE OR REPLACE PACKAGE populate_np_activities_pack AS
...
/
DROP PACKAGE BODY POPULATE_NP_ACTIVITIES_PACK;

CREATE OR REPLACE PACKAGE BODY populate_np_activities_pack AS

-- Populate table for resending NP_BEST transactions
-- inl TD4265 - not in use, but meanwhile kept for history
 PROCEDURE POPULATE_NP_BEST_RESEND_OLD IS
   ...
   from np_trx_detail@PROD nt,
        np_number_info@PROD np,
        np_order_data@PROD nd
 ...
 end POPULATE_NP_BEST_RESEND_OLD;
...
-- being ported to Netcom
 ...
 cursor c1 is
    ...
    where port_ind in ('Y','B','P','I')
      ...
      -- Prevent SMSes for IVER on Dec 22nd, 23rd and 24th EVERY year
      --and to_char(port_request_date, 'MM-DD') not in ('12-22', '12-23', '12-24')
      -- As of Dec 2015, Asle Ahlstrøm only want to stop SMSes on the 24th.
      --and to_char(port_request_date, 'MM-DD') NOT IN ('12-24')
      -- As Dec 24 is on a Monday in 2018, we need to block SMSes between 21-24.
      AND TO_CHAR(port_request_date, 'MM-DD') NOT IN ('12-21', '12-22', '12-23', '12-24')
 ...
 -- Populate table for sending welcome SMS to ported customers
  ...
  cursor c1 is
    SELECT    port_number, ni.int_order_id
       FROM   np_number_info@PROD ni,
              np_order_data@prod nd,
              subscriber@PROD s
       WHERE  port_ind in ('Y','B','P','I')
       and    number_type='P'
       AND    ni.effective_date = (SELECT MAX(effective_date)
                                   FROM   NP_NUMBER_INFO@PROD WHERE PORT_NUMBER = ni.PORT_NUMBER)
       ...
       AND    s.subscriber_no = 'GSM047' || ni.port_number
       AND    s.sub_status    = 'A'
       and not exists(select 1
                       from batch_np_activities bnp
...
-- Populate table for sending email to call center when no reply received on BEST(105) or ENDR(108) within 5 days.
-- inl 31.05.2010 TD4265 - Ninja should only send email and not resend BEST for these cases.
